...
> 2. 根据索引检索
>
> 索引提高检索效率的根本原因是缩小了扫描的范围



> 索引虽然可以提高检索效率，但是不能随意的添加索引，因为索引也是数据库当中的对象，也需要数据库不断地维护。是有维护成本的。比如，表中的数据经常被修改，这样就不适合添加索引，因为数据一旦修改，索引就需要重新排序，进项维护。



> 添加索引是给摸一个字段，或者说某些1字段添加索引
